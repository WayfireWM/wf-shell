...
void GvcCommon::default_stream_changed(GvcMixerControl *control, guint id, gpointer data)
{
    ...
    if (role == StreamRole::Sink)
    {
        ...
        instance->notify_sink_muted_signal =
            g_signal_connect(stream, "notify::is-muted", G_CALLBACK(default_stream_muted), data);
        instance->notify_sink_volume_signal =
            g_signal_connect(stream, "notify::volume", G_CALLBACK(default_stream_volume), data);

        default_stream_muted(instance->gvc_control, 0, data);
        default_stream_volume(instance->gvc_control, 0, data);
    } else
    {
